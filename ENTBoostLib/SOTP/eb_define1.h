...
typedef enum EB_MSG_TYPE
{
    ...
    , EB_MSG_RESET_ACCOUNT          = 0x114     // 重置账号
    
	, EB_MSG_USER_HEAD_CHANGE		= 0x121		// 用户修改头像资源
    , EB_MSG_BROADCAST_MESSAGE      = 0x122     // 广播消息
    , EB_MSG_GROUP_MEMBER_CHANGE    = 0x123     // 群成员资料变更
    , EB_MSG_ENT_GROUP_INFO_VERSION = 0x124     // 企业部门资料版本
    , EB_MSG_GROUP_MEMBER_VERSION   = 0x125     // 部门成员资料版本
    ...
    , EB_MSG_EMOTION_COUNT          = 0x139     // 表情资源汇总
    ...
    , EB_MSG_UPDATE_CONTACT         = 0x145     // 新增或更新好友
    , EB_MSG_UPDATE_CONTACT_GROUP   = 0x146     // 新增或更新好友分组
    , EB_MSG_DELETE_CONTACT_GROUP   = 0x147     // 删除好友分组
    , EB_MSG_UPDATE_CONTACT_GROUP2              // 更新好友分组(新版本)
    , EB_MSG_DELETE_CONTACT_GROUP2              // 删除好友分组(新版本)
...
{
    EB_RICH_SUB_TYPE_JPG = 0      //JPG图片
    ,EB_RICH_SUB_TYPE_AUDIO = 11   //语音
    ,EB_RICH_SUB_TYPE_MAP_POS = 21 //地图位置
    ,EB_RICH_SUB_TYPE_CARD_INFO = 22 //电子名片
    ,EB_RICH_SUB_TYPE_USER_DATA = 200 //用户自定义消息子类型(包括200及以上)
} EB_RICH_SUB_TYPE;
...
typedef enum EB_MANAGER_LEVEL
{
    ...
    , EB_LEVEL_FORBID_SPEECH    = 0x1000    // 禁言
} EB_MANAGER_LEVEL;
...
{
	EB_MAT_SUCCESS      = 0 // 接收成功
	, EB_MAT_ERROR      = 1 // 接收失败
	, EB_MAT_CANCEL     = 2 // 取消发送(接收)消息(文件)
	, EB_MAT_REQUEST    = 3 // 请求开始接收[文件]
    , EB_MAT_RETRACT    = 4 // 请求撤回消息，只支持撤回2分钟内消息
    
    , EB_MAT_COLLECT_PERSON   = 6 // 请求将消息添加到“个人收藏”
    , EB_MAT_COLLECT_GROUP    = 7 // 请求将消息添加到“群收藏”
    
    , EB_MAT_SAVE_OFFLINE_FILE  = 10 // 保存离线文件到云盘[文件]
    , EB_MAT_RESEND             = 11 // 请求重新发送一次消息
} EB_MSG_ACK_TYPE;
...
typedef enum EB_CALL_ACK_TYPE
{
	...
	, EB_CAT_ACCEPT     = 1 // 接受
	, EB_CAT_REJECT     = 2 // 拒绝
    , EB_CAT_OFFLINE    = 3 // 用户离线
    , EB_CAT_TIMEOUT    = 4 // 呼叫超时
} EB_CALL_ACK_TYPE;
