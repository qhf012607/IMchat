 ...
 * @param richSubType 富文本信息子类型(EB_RICH_SUB_TYPE_JPG =图片, EB_RICH_SUB_TYPE_AUDIO=语音, EB_RICH_SUB_TYPE_MAP_POS=地图位置, EB_RICH_SUB_TYPE_CARD_INFO=电子名片),各种不同子类型不可以同一条聊天信息中混发
 * @param callId 会话编号
...
/**获取全部联系人信息列表
 * @return 联系人信息列表(EBContactInfo实例)
 */
 ...
 * @return 联系人信息列表(EBContactInfo实例)
 */
- (NSArray*)contactInfosWithGroupId:(uint64_t)groupId;
