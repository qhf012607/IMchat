 ...
 * @param target 目标
 * @param action 回调方法
 * @param viewHeight 视图高度，如填入负数表示忽略
 * @param clipsToBounds 是否切边(限制内部显示内容边界)
 */
//- (void)addHeaderWithTarget:(id)target action:(SEL)action viewHeight:(CGFloat)viewHeight clipsToBounds:(BOOL)clipsToBounds;
 ...
 * @param clipsToBounds 是否切边(限制内部显示内容边界)
 */
- (void)addHeaderWithDelegate:(id)delegate viewHeight:(CGFloat)viewHeight clipsToBounds:(BOOL)clipsToBounds;
 ...
 *  @param action 回调方法
 */
- (void)addFooterWithDelegate:(id)Delegate action:(SEL)action;
