...
@interface TalkViewController () <UIScrollViewDelegate, UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SETextViewDelegate, ChatCellDelegate, StampInputViewDelegate, AudioToolkitDelegate, FilesBrowserDelegate, AQSViewControllerDelegate, MJRefreshDelegate>
{
...
//    [self.talkTableView addHeaderWithTarget:self action:@selector(headerRereshing) viewHeight:0.0 clipsToBounds:YES];
    [self.talkTableView addHeaderWithDelegate:self viewHeight:0.0 clipsToBounds:YES];
...
- (IBAction)showCameraPicker:(id)sender
{
    ...
    if(![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        UIAlertView* alertView = [[UIAlertView alloc] initWithTitle:@"无法拍照" message:@"没有相机" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) {
        ...
        [self performSelector:@selector(sendImageData:) withObject:data afterDelay:0.2];
...
//                    EBChatAudio* chatAudio = (EBChatAudio*)chatDot;
                    ...
                    break;
                default:
                    NSLog(@"Not supported chat type %i", chatDot.type);
                    break;
            }
        }
    }
...
- (void)stampInputView:(StampInputView *)stampInputView stampTaped:(StampButton *)button
{
    ...
    if (emotion.dynamicFilepath)
        ...
        image = [UIImage imageNamed:@"loading_emotion"];
    image = [image scaleToSize:CGSizeMake(24.0, 24.0)];
