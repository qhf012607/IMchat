...
- (void)updateTree:(TableTree*)tree usingMemberInfo:(EBMemberInfo*)memberInfo groupInfo:(EBGroupInfo*)groupInfo groupType:(RELATIONSHIP_TYPE)groupType onlineCountOfMembers:(NSInteger)onlineCountOfMembers isRemove:(BOOL)isRemove
{
    ...
    if (memberInfo) {
        ...
        if (memberInfo.uid == [ENTBoostKit sharedToolKit].accountInfo.uid)
            ...
            memberNode.isHiddenTalkBtn = YES; //NO;
...
{
    __weak typeof(self) safeSelf = self;
    NSString* nodeId = [NSString stringWithFormat:@"%llu", groupInfo.depCode];
    
    //企业部门
    if (groupInfo.entCode) {
        [BlockUtility performBlockInMainQueue:^{
            //在“我的部门“删除对应节点
            [safeSelf.helper.myDepartmentTree removeNodeWithId:nodeId updateParentNodeLoadedState:NO];
            //在"企业架构"删除对应节点
            [safeSelf.helper.enterpriseTree removeNodeWithId:nodeId updateParentNodeLoadedState:YES];
            //更新企业架构视图标题
            [safeSelf.helper updateSelectedViewLabelOfEnterpriseView];
        }];
    }
    //个人群组
        ...
        [BlockUtility performBlockInMainQueue:^{
            [safeSelf.helper.personalGroupTree removeNodeWithId:nodeId updateParentNodeLoadedState:YES];
        }];
    }
...
- (void)handleUpdateContact:(EBContactInfo *)contactInfo isChangedGroup:(BOOL)isChangedGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    if (!isChangedGroup)
        [self updateTree:self.helper.contactTree usingContactInfo:contactInfo isRemove:NO];
    else {
...
- (void)handleUpdateContactGroup:(EBContactGroup *)contactGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    [BlockUtility performBlockInMainQueue:^{
        [self.helper reloadContactTree];
    }];
...
- (void)handleDeleteContactGroup:(EBContactGroup *)contactGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    [BlockUtility performBlockInMainQueue:^{
        [self.helper reloadContactTree];
    }];
