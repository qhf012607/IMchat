...
//        UIImage *image = [UIImage imageNamed:imageData.name];
        if (imageData.scaleImage) {
            CGRect imagePosition = imageData.imagePosition;
            if (imageData.imageType==CHAT_IMAGE_TYPE_RESOURCE) {
                //避免表情不是正方形
                CGFloat min = fminf(imageData.imagePosition.size.width, imageData.imagePosition.size.height);
                imagePosition = CGRectMake(imageData.imagePosition.origin.x, imageData.imagePosition.origin.y, min, min);
            }
            CGContextDrawImage(context, imagePosition, imageData.image.CGImage);//imageData.scaleImage.CGImage);
        }
    }
...
- (void)userSingleTapGestureDetected:(UITapGestureRecognizer *)recognizer
{
    ...
    if (self.state == ChatViewStateNormal) {
        ...
        for (CoreTextImageData * imageData in self.data.imageArray) {
            ...
            //NSLog(@"+++%@", NSStringFromCGSize(imageRect.size));
