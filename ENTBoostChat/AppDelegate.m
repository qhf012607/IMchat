...
//新增或变更好友的通知事件
- (void)onUpdateContact:(EBContactInfo *)contactInfo isChangedGroup:(BOOL)isChangedGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    [self.tabBarController.relationshipController handleUpdateContact:contactInfo isChangedGroup:isChangedGroup fromUid:fromUid fromAccount:fromAccount vCard:vCard];
}

- (void)onUpdateContactGroup:(EBContactGroup *)contactGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    [self.tabBarController.relationshipController handleUpdateContactGroup:contactGroup fromUid:fromUid fromAccount:fromAccount vCard:vCard];
}

- (void)onDeleteContactGroup:(EBContactGroup *)contactGroup fromUid:(uint64_t)fromUid fromAccount:(NSString *)fromAccount vCard:(EBVCard *)vCard
{
    [self.tabBarController.relationshipController handleDeleteContactGroup:contactGroup fromUid:fromUid fromAccount:fromAccount vCard:vCard];
}
